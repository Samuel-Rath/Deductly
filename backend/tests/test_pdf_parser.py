...
Covers the state-machine parser, transaction normalisation, date/amount
...
class TestPDFParserInternals:
    ...
    @pytest.mark.parametrize("date_str", [
        "not a date",
        "2024-01-15",      # ISO format isn't supported
    ...
    # --- _extract_amount_from_description --------------------------------
    ...
    def test_is_disclaimer_line_single_phrase_is_not_disclaimer(self, parser):
        # One keyword is not enough — two or more must match
...
# ---------------------------------------------------------------------------
# State machine tests — parse_with_state_machine directly
# ---------------------------------------------------------------------------
...
class TestStateMachine:
    ...
    def test_skips_disclaimer_lines_in_description(self, parser):
        ...
        txns = parser._parse_with_state_machine(text)
        assert len(txns) == 1
        # The disclaimer line must not pollute the description
    ...
    def test_date_only_no_amount_is_dropped(self, parser):
        # A date without a following amount should not produce a transaction
        text = "15/01/2024 SOME LINE WITH NO MONEY\n"
        txns = parser._parse_with_state_machine(text)
        assert txns == []

    def test_detect_recurring_is_invoked(self, parser):
        # Two identical-merchant transactions a month apart with the same
        # amount should surface through the recurring detector without error.
        text = (
        ...
        txns = parser._parse_with_state_machine(text)
        # recurring detection runs in parse(), but verify we at least get
        # all three transactions out of the state machine
        assert len(txns) == 3


# ---------------------------------------------------------------------------
# Integration tests — parse() against real PDFs produced by reportlab
# ---------------------------------------------------------------------------
...
class TestPDFParserIntegration:
    ...
    def test_parse_table_based_pdf(self, parser):
        ...
        txns = parser.parse(pdf)
        # Table-based PDFs may or may not yield transactions depending on
        # how reportlab lays them out; but if any do come back they must be
        # valid NormalisedTransaction objects.
        for t in txns:
